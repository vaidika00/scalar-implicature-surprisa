...
def word_surprisal(sentence: str, target_word: str) -> float:
...
def run_all(items):
    ...
    for item_id, condition, sentence, crit_word in items:
        bits = word_surprisal(sentence, crit_word)
        rows.append({
            "item_id": item_id,
            "condition": condition,
            "sentence": sentence,
...
if __name__ == "__main__":
    df = run_all(SENTENCE_ITEMS)
    print(df[["item_id", "condition", "surprisal_bits"]])
    df.to_csv("surprisal_results.csv", index=False)

    # Paired comparison: upper-bound vs lower-bound surprisal per item
    ...
    print(f"\nPaired t-test: t={t:.3f}, p={p:.3f}")

    # With only 4 items, also report Wilcoxon as a nonparametric check
    try:
    ...
    # Break down by trigger word too, since "some" and "or" items come
    # from different experiments with different human RT benchmarks
